...
plt.figure(1, figsize=(7,4)) # figsize parameter added to fit all data in the 3rd subplot
plt.subplot(1,3,1)
...
'''
#Some additional code written by us to better understand the performance of our model,
#the resulting csv has been used in the jupyter notebook.
...
cols_3c = ['dist_type', 'hist_type', 'num_bins', 'num_correct', 'num_correct_prop']

model_images = model_images
query_images = query_images

for dist_type in ['l2', 'intersect', 'chi2']:
    for hist_type in ['rg', 'rgb', 'grayvalue', 'dxdy']:
        for num_bins in [10, 30, 50]:
            
            print(dist_type, ' ' , hist_type, ' ', num_bins)
            
            [best_match, D] = match_module.find_best_match(model_images, query_images, dist_type, hist_type, num_bins)
            #match_module.show_neighbors(model_images, query_images, dist_type, hist_type, num_bins)
            
            num_correct = sum( best_match == range(len(query_images)) )
            correct_prop = 1.0 * num_correct / len(query_images)
                            
            print('number of correct matches: %d (%f)\n'% (num_correct, 1.0 * num_correct / len(query_images)))
            
            row = pd.DataFrame(data = [[dist_type, hist_type, num_bins, num_correct, correct_prop]],
                               columns = cols_3c,
                               index = [0])
            
            results_3c = results_3c.append(row)
            results_3c.reset_index(inplace = True, drop = True)


results_3c.reset_index(inplace = True, drop = True)
#results_3c.to_csv('results_3c.csv', header= True)

# End of the additional code for exercise 3.
'''
...
plt.title(f'RGB histograms- bins {num_bins//2}')
plt.show()
